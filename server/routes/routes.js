...
router.post('/', async (req, res) => {
  try {
    ...
    const id = uuidv4();
    await db.execute(
      'INSERT INTO routes (id, map_id, name, points, color, group_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [id, req.params.mapId, name, JSON.stringify(points), color, group_id, new Date().toISOString()]
    );
...
router.put('/:routeId', async (req, res) => {
  try {
    ...
    const [rows] = await db.execute(
      'SELECT * FROM routes WHERE id = ? AND map_id = ?',
      [req.params.routeId, req.params.mapId]
    );
...
router.delete('/:routeId', async (req, res) => {
  try {
    const [result] = await db.execute(
      'DELETE FROM routes WHERE id = ? AND map_id = ?',
      [req.params.routeId, req.params.mapId]
    );
